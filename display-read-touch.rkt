...
;; - supports only partially touch-screens, text editor boxes are not supported on touch screens e.g.
;;   display-read and display-read-number
...
(require "big-crunch.rkt")           ; comment out in WeScheme (remove big-crunch from code)
; (require bootstrap2012/bootstrap-tilt-teachpack) ; WeScheme
...
(define (display-value item value)
  (selector-choices (big-bang/big-crunch (make-selector value 0 (item->image item) #t)                       
...
;; render-keys : Boolean Boolean Boolean Boolean Boolean -> Image
(define A-COLOR "blue")

;; make-arrow : Number Color -> Image
;; angle: 0 right
;; angle: 90 up
;; angle: 180 left
;; angle: -90 down
(define (make-arrow angle color)
  (rotate angle (beside (rectangle 20 10 "solid" color)
                        (rotate -90 (triangle 24 "solid" color)))))

(define (make-ok color size)
  (overlay (circle size "outline" "black")
           (text "OK" (- size 2) "black")
           (circle size 120 color)))

(define SPACE (square 10 0 "transparent"))

(define (render-keys left right up down ok)
  (cond [(and left right up down ok)
         (above (make-arrow 90 A-COLOR)
                SPACE
                (beside (make-arrow 180 A-COLOR) SPACE (make-ok A-COLOR 15) SPACE (make-arrow 0 A-COLOR))
                SPACE
                (make-arrow -90 A-COLOR))]
        [(and up down ok)
         (above (make-arrow 90 A-COLOR)
                SPACE
                (beside (make-arrow 180 "transparent") SPACE (make-ok A-COLOR 15) SPACE (make-arrow 0 "transparent"))
                SPACE
                (make-arrow -90 A-COLOR))]
        [(and ok (not up)(not down)(not left)(not right))
         (above (make-arrow 90 "transparent")
                SPACE
                (beside (make-arrow 180 "transparent") SPACE (make-ok A-COLOR 18) SPACE (make-arrow 0 "transparent"))
                SPACE
                (make-arrow -90 "transparent"))]
        [else  (above (make-arrow 90 "transparent")
                      SPACE
                      (beside (make-arrow 180 "transparent") SPACE (make-ok "transparent" 15) SPACE (make-arrow 0 "transparent"))
                      SPACE
                      (make-arrow -90 "transparent"))]))
...
(define (render-values s)
  (above (render-img (selector-img s))
         ...
         ;; added soft keys
         (render-keys #f #f #f #f #t)))

;; render-info : Info -> Image
(define (render-info i)
         (if (info-timer? i)
             (render-img (render-value (info-item i)))
             ;; added soft keys
             (above (render-img (render-value (info-item i)))
                    (render-keys #f #f #f #f #t))))
...
(define A-KEY 46)
(define 1/2-OK-KEY 20)

;; mouse->key : Window/Selector/Info Number Number -> Key/false
(define (mouse->key a x y)
  (let* [(img (if (selector? a)
                  (render-selector a)
                  (render-just-image a)))
         (i-width (image-width img))
         (i-heigth (image-height img))
         (x0 (- (/ i-width 2) A-KEY 1/2-OK-KEY))
         (x1 (- (/ i-width 2) 1/2-OK-KEY))
         (x2 (+ (/ i-width 2) 1/2-OK-KEY))
         (x3 (+ (/ i-width 2) A-KEY 1/2-OK-KEY))
         (y0 i-heigth)
         (y1 (- y0 A-KEY))
         (y2 (- y0 A-KEY (* 2 1/2-OK-KEY)))
         (y3 (- y0 (* 2 (+ A-KEY 1/2-OK-KEY))))]
  (cond [(and (< x0 x x1)
              (< y2 y y1))
         "left"]
        [(and (< x1 x x2)
...
;; handle-mouse : Window/Selector/Info Number Number Mouse-event -> Window/Selector/Info
(define (handle-mouse a x y event)
  (if (equal? event "button-down")
      (let [(pressed-key (mouse->key a x y))]
        (cond [(false? pressed-key) a]
              [(selector? a) (handle-key-selector a pressed-key)]
              [(info? a) (handle-key-just-image a pressed-key)]
              [else a]))
      a))
